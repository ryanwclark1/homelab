...
const details = () => ({
  id: 'Tdarr_Plugin_MC93_Migz1FFMPEG_AV1',
  Stage: 'Pre-processing',
  Name: 'Migz Transcode Using Intel Arc GPU & FFMPEG to AV1',
  Type: 'Video',
  Operation: 'Transcode',
  Description: `Files not in AV1 will be transcoded into AV1 using Intel Arc GPU with ffmpeg.
                  Settings are dependant on file bitrate
                  Working by the logic that AV1 can support the same ammount of data at half the bitrate of H264.
                  QSV compatable GPU required.
                  This plugin will not skip any files that are not in AV1 codec.`,
  Version: '1.0',
...
const plugin = (file, librarySettings, inputs, otherArguments) => {
  ...
  // Logic of AV1 can be half the bitrate as H264 without losing quality.
  // eslint-disable-next-line no-bitwise
  ...
  for (let i = 0; i < file.ffProbeData.streams.length; i++) {
    ...
    if (codec_type === 'video') {
      ...
      // Check if codec of stream is AV1 AND check if file.container matches inputs.container.
      ...
      if (
        ...
        response.processFile = false;
        response.infoLog += `File is already AV1 & in ${inputs.container}. \n`;
        return response;
      }
      // Check if codec of stream is AV1
      ...
      if (
        ...
        response.infoLog += `File is AV1 but is not in ${inputs.container} container. Remuxing. \n`;
  ...
  // Set the preset for AV1 encoding with QSV
  response.preset = `${genpts}, -map 0 -c:v av1_qsv -rc 1 ${bitrateSettings} -c:a copy -c:s copy -max_muxing_queue_size 9999 ${extraArguments}`;
  response.processFile = true;
  response.infoLog += 'File is not AV1. Transcoding to AV1. \n';
  return response;
};
